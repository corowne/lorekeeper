            ...
            <div class="row mb-3">
                <div class="col-md-7">
                    <div class="text-center">
                        <a href="{{ $image->canViewFull && file_exists( public_path($image->imageDirectory.'/'.$image->fullsizeFileName)) ? $image->fullsizeUrl : $image->imageUrl }}" data-lightbox="entry" data-title="{{ $character->fullName }} [#{{ $image->id }}] {{ $image->canViewFull && file_exists( public_path($image->imageDirectory.'/'.$image->fullsizeFileName)) ? ' : Full-size Image' : ''}}">
                            <img src="{{ $image->canViewFull && file_exists( public_path($image->imageDirectory.'/'.$image->fullsizeFileName)) ? $image->fullsizeUrl : $image->imageUrl }}" class="image" />
                        </a>
                    </div>
                    @if($image->canViewFull && file_exists( public_path($image->imageDirectory.'/'.$image->fullsizeFileName)))

@if (!$forum->is_locked)
    <div class="float-right mb-2">
        <a class="btn btn-primary" href="{{ url('forum/' . $forum->id . '/new') }}">New Thread</a>
    </div>
@endif
...
@if ($forum->accessibleSubforums->count())
    <div class="row no-gutters mb-2" style="clear:both;">
        ...
        @foreach ($forum->accessibleSubforums as $board)
            <div class=" {{ $loop->even ? 'px-2' : '' }} col-md-4">
                <div class="card p-2 h-100">
                    {!! $board->displayName !!}
                    @if ($board->accessibleSubforums->count())
                        <p class="mb-0" style="font-size: 0.8em;">Sub-Forums: {!! implode(', ', $board->accessibleSubforums->pluck('displayName', 'id')->toArray()) !!}</p>
                    @endif
                </div>
            </div>
        @endforeach
                        ...
                        <div class="col-6 col-md-3 my-auto">
                            <p class="mb-0">
                                @if (isset($comment->latestReply))
                                    <span class="d-none d-md-inline">Latest reply by {!! $comment->latestReply->commenter->displayName !!} - </span>{!! pretty_date($comment->latestReply->updated_at) !!}
                                @else
                                    No Replies
                                @endif
                            </p>

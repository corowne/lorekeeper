    ...
    @case('MultiItem')
        ...
        <p class="mb-0">
            @foreach($ingredient->ingredient as $key => $ing)
                @if(isset($ing->image_url))<img class="small-icon" src="{{ $ing->image_url }}">@endif <strong>{!! $ing->displayName !!}</strong>{{ ($key < $ingredient->ingredient->count()-1) && ($ingredient->ingredient->count() > 2) ? ', ' : '' }}{{ ($key == $ingredient->ingredient->count()-2) && ($ingredient->ingredient->count() > 1) ? ' and ' : '' }}
            @endforeach
        </p>
        @break
    @case('Category')

...
@if($trade->comment)
    <h2>{!! $trade->sender->displayName !!}'s Comment</h2>
    <div class="card mb-3">
        <div class="card-body">
            {{ nl2br(htmlentities($trade->comment)) }}
        </div>
    </div>
@endif
...
@if(Auth::user()->id == $trade->sender_id || Auth::user()->id == $trade->recipient_id)
    <div class="text-right">
        @if(!$trade->isConfirmable)
                {!! add_help('Both parties must confirm their offer before you can confirm this trade.') !!}
        ...
        @else 
            ...
            @endif
        @endif
        <a href="#" class="btn btn-outline-danger">Cancel Trade</a>
    </div>
@endif

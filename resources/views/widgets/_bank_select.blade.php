<h3>{{ $owner->lotType == 'User' && $owner->id == Auth::user()->id ? 'Your' : $owner->name . '\'s' }} Bank <a class="small inventory-collapse-toggle collapse-toggle" href="#{{ strtolower($owner->logType) }}Bank-{{ $owner->id }}" data-toggle="collapse">Show</a></h3>
<div class="collapse show" id="{{ strtolower($owner->logType) }}Bank-{{ $owner->id }}">

...
class CommentController extends Controller {
    ...
    public function store(Request $request, $model, $id) {
        ...
        $comment->comment = config('lorekeeper.settings.wysiwyg_comments') ? $request->message : parse($request->message);
    ...
    public function update(Request $request, Comment $comment) {
        ...
        $comment->edits()->create([
            ...
            'data'       => json_encode([
                'action'      => 'edit',
                'old_comment' => config('lorekeeper.settings.wysiwyg_comments') ? $comment->comment : parse($comment->comment),
                'new_comment' => config('lorekeeper.settings.wysiwyg_comments') ? $request->message : parse($request->message),
            ]),
        ]);

        $comment->update([
            'comment' => config('lorekeeper.settings.wysiwyg_comments') ? $request->message : parse($request->message),
        ]);
    ...
    public function reply(Request $request, Comment $comment) {
        ...
        $reply->comment = config('lorekeeper.settings.wysiwyg_comments') ? $request->message : parse($request->message);

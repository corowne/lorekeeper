...
class UserController extends Controller
{
    ...
    public function postUserBasicInfo(Request $request, $name)
    {
        ...
        elseif (!Auth::user()->canEditRank($user->rank)) {
            ...
            if(!logAdminAction($staff, 'Editted User', 'Editted '.$user->displayname)) { flash('Failed to log admin action.')->error(); return redirect()->back();}
    ...
    public function postUserAlias(Request $request, $name, $id)
    {
        ...
        if(!$user) flash('Invalid user.')->error();
            ...
            if(!logAdminAction($staff, 'Editted User', 'Cleared '.$user->displayname.'\' alias')) { flash('Failed to log admin action.')->error(); return redirect()->back(); }
    ...
    public function postUserBirthday(Request $request, $name)
    {
        ...
        if(!logAdminAction($staff, 'Editted User', 'Editted '.$user->displayname.' birthday')) { flash('Failed to log admin action.')->error(); return redirect()->back();}

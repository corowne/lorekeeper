    ...
    public function createSubmission($data, $user)
    {
        ...
        try {
            ...
            foreach($characters as $c) 
            {
                ...
                if(isset($data['currency_id'][$c->id]) && isset($data['quantity'][$c->id]))
                {
                    foreach($data['currency_id'][$c->id] as $key => $currency)
                    {
                        if($data['quantity'][$c->id][$key]) addAsset($assets, $currencies[$currency], $data['quantity'][$c->id][$key]);
                    }
                }

        ...
        try {
            ...
            if($data['quantity'] < 0)
                foreach($users as $user) {
                    if(!$this->logAdminAction($staff, 'Currency Debit', 'Debited '.$data['quantity'].' '.$currency->displayName.' from '.$user->displayname)) throw new \Exception("Failed to log admin action.");
                ...
                foreach($users as $user) {
                    if(!$this->logAdminAction($staff, 'Currency Grant', 'Granted '.$data['quantity'].' '.$currency->displayName.' to '.$user->displayname)) throw new \Exception("Failed to log admin action.");

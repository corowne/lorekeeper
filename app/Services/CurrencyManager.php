        ...
        try {
            ...
            if($data['quantity'] < 0)
                foreach($users as $user) {
                    if(!logAdminAction($staff, 'Currency Debit', 'Debitted '.$data['quantity'].' '.$currency->displayName.' from '.$user->displayname)) throw new \Exception("Failed to log admin action.");
